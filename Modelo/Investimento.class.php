...
class Investimento implements IModelo {
	...
	public function buscar($obj){
		...
		$retorno = $this->db->selectInClass($query, $params, "Investimento");
		
		return $retorno[0];
	}
	
	public function ListarPorNome($obj){
		$query = "select * from investimento where descricao = :descricao";
	...
	function listar($numeroRegistros,$pagina,$order){
		$query = "SELECT * FROM Investimento ORDER BY :order LIMIT :limit OFFSET :offset ";
		$params[":order"] = $order;
		$params[":limit"] = $numeroRegistros;
		$params[":offset"] = ($pagina - 1) * $numeroRegistros;
		$list = $this->db->selectInClass($query, $params, "Investimento");
		/*$usuarioList = array();
		$count = 0;
	...
	function Insert($obj){
		$query = "insert into investimento (
			            id, descricao, status)
			    select COALESCE(MAX(id),0) + 1, :nome, :potencia, :valor, :tamanho, :status from investimento";
